...
function VPrediction:GetCurrentWayPoints(object)
	...
	if (GetBuildDate() == "Apr  9 2014") and TESTVERSION then
...
function VPrediction:GetBestCastPosition(unit, delay, radius, range, speed, from, collision, spelltype)
	...
	if IsFromMyHero then
		...
		if (spelltype == "circular" and (GetDistanceSqr(from, Position) >= (range + self:GetHitBox(unit) + radius)^2)) then
			HitChance = 0
		end
		if (GetDistanceSqr(from, CastPosition) > range^2) then
			HitChance = 0
		end
	end
...
function VPrediction:GetConeAOECastPosition(unit, delay, angle, range, speed, from)
	...
	if MaxHit > 1 then
		...
		return Vector(from) + range * (((Vector(p1) + Vector(p2)) / 2) - Vector(from)):normalized(), mainHitChance, MaxHit
	else
